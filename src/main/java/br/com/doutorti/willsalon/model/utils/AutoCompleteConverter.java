...
public class AutoCompleteConverter implements Converter, Serializable {
	...
	public Object getAsObject( FacesContext context, UIComponent component, String value ) {
		if ( value == null || value.trim().isEmpty() || value.equals( "{" ) || value.equals( "\"\"" ) || !value.startsWith( "{" ) )
			return null;
		...
		if ( jsonResult.has( "birthDate" ) ) {
			...
			try {
				Date birthDate = new SimpleDateFormat( "MMM-dd-yyyy" ).parse( birthDateString );
				jsonResult.put( "birthDate", new SimpleDateFormat( "yyyy-MM-dd" ).format( birthDate ) );
			} catch ( ParseException e ) {
				e.printStackTrace();
			}
		}

		return new Gson().fromJson( jsonResult.toString(), (Class<?>) component.getAttributes().get( ATTRIBUTE_CLASS ) );
	}

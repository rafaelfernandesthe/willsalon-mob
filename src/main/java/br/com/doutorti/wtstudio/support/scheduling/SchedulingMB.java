...
public class SchedulingMB extends BaseBeans {
	...
	private static final Logger logger = Logger.getLogger(SchedulingMB.class);
	...
	public void loadSchedulingRepeatedList(Long id) {
		...
		schedulingRepeatedList = schedulingRepository
				.findByWasRepetitionAndHour(getItemRowSelected()
						.getInitialDate(), getItemRowSelected().getClient()
						.getId(), getItemRowSelected().getEmployee().getId());
		this.setSchedulingShow(this.schedulingRepository.findOne(id));
		RequestContext.getCurrentInstance().execute(
				"PF('dialog_delete').show()");
	...
	public void deleteMany() {
		if (this.schedulingRepeatedList != null
				&& !this.schedulingRepeatedList.isEmpty()) {
	...
	public void setSchedulingRepeatedList(
			List<SchedulingEntity> schedulingRepeatedList) {
